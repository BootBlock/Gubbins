...
| **58** ✅ | Backlog (developer-chosen) — fresh investigation (project budgeting) | **§4 project budgeting** (a fresh-investigation pick layered on the existing §4 BOM Costing: the app *estimated* a project's cost (live/snapshot `getCosting`) but had no notion of an *allotted* budget or *actual* spend). Additive **v20** nullable `projects.budget` + two new synced tables — `project_budget_categories` (optional named sub-budgets) and `project_expenses` (the manual spend ledger; `category_id` ON DELETE SET NULL, cascade on project delete). Spend has two lanes: **committed** = `Σ received_qty × unit cost` and the full **estimate** are *derived projections* over the BOM under the costing mode (never stored — the Phase-20 "derive, never store a counter" seam), and **manual** = the expense ledger (shipping/labour/tools the BOM can't capture). The pure `features/projects/budget.ts` (`summariseBudget`/`budgetStatus`/category rollup) turns the repo's raw `getBudget` aggregates into spent/remaining/projected/status (OK/WARN/OVER); the `withBudget` `ProjectRepository` mixin adds `getBudget`, expense + budget-category CRUD (tombstoned) and the cross-project `listBudgetAlerts` feed. UI: a `BudgetSection` on the project detail (token-driven `BudgetMeter`, breakdown, warn/over badge, `SetBudgetDialog`, `BudgetCategoryEditor`, `ExpenseLedger`/`ExpenseDialog`), a dashboard **"Budget alerts"** widget, and a Settings **budget-warn-%** control (`budgetWarnPercent` Tier-2 pref, default 80, `clampBudgetWarnPercent`, the canonical "fixed constant → Tier-2 pref" pattern). Budget summary also flows into the §4.5 project-scope vault master note. Sync: both tables in `SYNC_TABLES` + `FK_REFS` (project_id cascade guard; category_id null-out) with round-trip / cascade / un-categorise tests. **`user_version` is now 20.** No dependency change; precache 3000.93 KiB (reporter only). **`build:extension` NOT re-run** (no §9 / `extension/` edit). Remaining open items are still triggered Backlog entries (below). |
...
in-app scanner recognises and selects). Plan doc + full detail:
...
**no schema change, `user_version` stays 1**. Plan doc + full detail:
...
`user_version` stays 1.** Plan doc: `docs/todo/done/calendar_2026-06-30.md`.
...
`user_version` stays **1**. Plan doc: `docs/todo/done/bulk-edit_2026-06-30.md`.
...
stays 1.** Plan doc: `docs/todo/done/data-hygiene_2026-06-30.md`.
...
Plan doc: `docs/todo/done/asset-booking_2026-06-30.md`. Tracked in `feature-gap-audit-2026-06-30c`.
...
Plan doc: `docs/todo/done/spend-analytics_2026-06-30.md`.
...
Plan doc: `docs/todo/done/activity-feed_2026-06-30.md`.
...
Plan doc: `docs/todo/done/supplier-price-history_2026-06-30.md`.
