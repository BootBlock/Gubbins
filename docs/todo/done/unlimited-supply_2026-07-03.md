...
> **Historical process note.** Implemented in its own worktree/session. Tick the `[ ]` boxes
> as work lands, append a one-paragraph **Outcome** note when it completes (mirroring
...
- [x] `is_unlimited` column + two CHECKs in the squashed `v1-initial`; snapshot regenerated;
      `user_version` still 1; CHECK-rejection test
- [x] `unlimited.ts` pure seam + tests; `reorder-policy` extended
- [x] mapper / types / `create` / `update` / `clone` round-trip the flag
- [x] `listLowStock`, BOM shortfall, consumption/reservation, reports honour unlimited
- [x] checkout blocked + cycle-count sheet excludes unlimited
- [x] Create/Edit toggle (DISCRETE-gated) + ∞ quantity display + ∞ badge (tokens, British English)
- [x] export/import + HA bridge (`quantity: null` + `is_unlimited: true`) round-trip
- [x] code review passed (mandatory pre-merge gate); PHASE_HANDOVER updated; Outcome note appended
...
**Shipped 2026-07-03.** Implemented exactly per the touch-list. `items.is_unlimited INTEGER NOT NULL
DEFAULT 0` + the two CHECKs were added to the squashed `v1-initial` (snapshot regenerated; `user_version`
still 1; no `SYNC_TABLES`/`FK_REFS` change — the additive synced column reconciles LWW for free). The pure
`src/features/inventory/unlimited.ts` seam (`isUnlimited` / `canSupply` / `consumptionLedgerDelta` /
`formatQuantityDisplay` / `UNLIMITED_GLYPH`) is the SSOT the UI, repositories and reports reuse; `reorder-policy`
gained an `isUnlimited` early-return (never low, shortfall 0). Repository/glue honour it everywhere: `listLowStock`
(+ `lowStockCount` / `listReorderShortfall`), the `getShoppingList` shortfall query, `inventoryValue`/`deadStock`,
the cycle-count sheet reads, and the checkout guard (blocked like UNTRACKED); `finaliseAssembly` logs `CONSUMED`
for an unlimited component but no longer soft-deletes it (an infinite source survives the build — the ledger
no-op). Clone, mapper, create, update and the catalogue import/export all round-trip the flag (blank CSV quantity
cell; `unlimited`/`is_unlimited` import header aliases; a non-DISCRETE `unlimited=true` row is rejected). UI: a
DISCRETE-gated "Unlimited supply" toggle with an `InfoHint` on Create + Edit; a `text-glyph-scan`-tinted **∞**
(first branch of the quantity ladder, no ± stepper) plus an ∞ `UnlimitedBadge` and a new `InfinityIcon`. HA bridge:
`quantity: null` + `isUnlimited: true` in JSON, blank CSV quantity cell, updated OpenAPI/`openapi.yaml`/CSDL/README
and a new `dto` test.

**Verification.** `tsc -p tsconfig.app.json` + `tsc -b` clean; `npm run build` clean; **2082** app unit tests +
**279** bridge tests pass (11 + 1 new). The Phase-82 UI was confirmed in a real browser (Edge) via a targeted
Playwright check (create unlimited → ∞ renders → no stepper). The mandatory **/code-review** gate ran; its single
finding — `/api/v1/search` serialising `quantity: null` against the non-nullable `ItemMatch` schema — was fixed by
widening `ItemMatch.quantity` to nullable (yaml regenerated).

**Discovered, re-scheduled (not dropped):** the full `browser-smoke.mjs` cascades from an unrelated, pre-existing
**"kiosk mode"** step that calls `selectOption(...)` on what is now a Foundry Select **combobox** (role
`combobox`) — it throws and blocks every later step. → **Fix the kiosk-mode (and any sibling combobox) smoke steps
to the click-open + click-option pattern in the next maintenance pass / Phase 83**, then re-run the full smoke from
the primary checkout.

## Continuation prompt

```
Phase 83 — Gubbins maintenance + next feature. First, in its own worktree, fix the pre-existing
browser-smoke drift discovered during Phase 82: `scripts/browser-smoke.mjs` cascades from the
"kiosk mode keeps the dashboard awake and contained" step, which calls `selectOption(...)` on
`data-testid="setting-kiosk-mode"` — now a Foundry Select combobox (role="combobox"), not a native
<select>. Convert that step (and audit sibling `selectOption` calls on Foundry comboboxes, e.g. any
settings/tracking selects that migrated) to the click-open + click-option pattern (see memory
[[foundry-select-combobox]]), then run the FULL `npm run test:e2e` browser smoke green from the
primary checkout (dev server on :5173). Then pick up the next backlogged item from
`docs/dev/deferred-features.md`. Follow the standing protocols: strict TDD, pure .ts logic split
from glue, design tokens only, British English; verify with `npx tsc -p tsconfig.app.json --noEmit`,
`npm run test:run`, `npm run build` (+ bridge tests if bridge/ is touched); run the mandatory
pre-merge /code-review gate; update PHASE_HANDOVER and append an Outcome note.
```
