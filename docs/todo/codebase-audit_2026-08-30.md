...
> **Status:** 🟢 ACTIVE — Phases 0–2 complete; Phase 3 (every other repository) is in progress.
...
| 3 | Every other repository | remaining `src/db/repositories/*Repository.ts` and their tests, `src/db/repositories/project/`, `permissions.enforcement.test.ts`, `wishlist.test.ts`, `revaluation.test.ts`, `item-relations.test.ts`, `tare-presets.test.ts`, `test-record.test.ts` | in progress | | | | |
