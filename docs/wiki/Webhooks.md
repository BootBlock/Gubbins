...
- **Blocked** — refused before anything was sent: either the address is on a private network the
  ...
  secret the bridge can't find gets a row the first time it would have delivered, so one that has
  quietly stopped doesn't just look idle and you don't have to send a test event to find out. That
  row names no event type, because no event got as far as being considered, and it repeats at most
  once an hour while the problem lasts.
