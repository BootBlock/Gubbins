...
> instead, and the print dialog says so above the preview. That short code needs a label roughly
> **30 mm wide or more**, so on the smallest sizes — the 30 × 15 mm labels, say — only a genuinely
> short name or part number prints as a barcode at all; anything longer leaves the barcode off
> entirely. Choose a wider label size, use fewer columns per sheet, or switch to a QR code. Either
