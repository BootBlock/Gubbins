### Gubbins wiki

**Getting started**
- [[Home]]
- [[Introduction]]
- [[Installing Gubbins|Installing-Gubbins]]
- [[Self-hosting with Docker|Self-Hosting-with-Docker]]
- [[Quick start|First-Run-and-Quick-Start]]
- [[Core concepts|Core-Concepts]]

**Inventory**
- [[Items]]
- [[Tracking modes|Tracking-Modes]]
- [[Locations & stock|Locations-and-Stock]]
- [[Location photos & regions|Location-Photos-and-Regions]]
- [[Batches & lots|Batches-and-Lots]]
- [[Variants & SKUs|Variants-and-SKUs]]
- [[Kits & bundles|Kits-and-Bundles]]
- [[Tags, attachments & related|Tags-Attachments-and-Related-Items]]
- [[Custom fields & capabilities|Custom-Fields-and-Capabilities]]
- [[Low stock & gauges|Low-Stock-and-Gauges]]
- [[Counting by weight|Counting-by-Weight]]
- [[Container weights|Container-Weights]]
- [[Bulk edit & clone|Bulk-Edit-and-Clone]]
- [[Inventory views|Inventory-Views]]

**Asset lifecycle**
- [[Warranty & depreciation|Warranty-and-Depreciation]]
- [[Current value & revaluation|Current-Value-and-Revaluation]]
- [[Maintenance & servicing|Maintenance-and-Servicing]]
- [[Test & calibration records|Test-and-Calibration-Records]]
- [[Condition grading|Condition-Grading]]
- [[Cycle counts & audit day|Cycle-Counts-and-Audit-Day]]
- [[Insurance & estate schedule|Insurance-and-Estate-Schedule]]

**People, loans & bookings**
- [[Contacts]]
- [[Loans|Loans-Check-Out-and-In]]
- [[Bookings]]

**Search**
- [[Search overview|Search-Overview]]
- [[Visual query builder|Visual-Query-Builder]]
- [[Text query syntax|Text-Query-Syntax]]
- [[Natural-language search|Natural-Language-Search]]
- [[Saved searches & favourites|Saved-Searches-and-Favourites]]

**Scanning & labels**
- [[Camera scanning|Camera-Scanning]]
- [[Continuous scan & batch actions|Continuous-Scan-and-Batch-Actions]]
- [[QR codes & label printing|QR-Codes-and-Label-Printing]]
- [[Receipt & label OCR|Receipt-and-Label-OCR]]

**Purchasing & projects**
- [[Projects & BOM|Projects-and-BOM]]
- [[Budgets]]
- [[Purchase orders|Purchase-Orders]]
- [[Reorder & shopping list|Reorder-and-Shopping-List]]
- [[Wishlist]]
- [[Supplier parts & price history|Supplier-Parts-and-Price-History]]
- [[Suppliers]]

**Reports & insights**
- [[Reports overview|Reports-Overview]]
- [[Valuation & spend|Valuation-and-Spend]]
- [[ABC, turnover & aging|ABC-Turnover-and-Aging]]
- [[Sales & margin|Sales-and-Margin]]
- [[Data hygiene|Data-Hygiene]]
- [[Parts catalogue|Parts-Catalogue]]

**Sales & disposals**
- [[Sales & disposals|Sales-and-Disposals]]

**Alerts, activity & agenda**
- [[Activity log|Activity-Log]]
- [[Alerts]]
- [[Upcoming agenda|Upcoming-Agenda]]
- [[Reminder notifications|Reminder-Notifications]]

**Companion extension**
- [[Extension setup|Companion-Extension-Setup]]
- [[Scraping supplier data|Scraping-Supplier-Data]]

**Data, sync & resilience**
- [[How your data is stored|How-Your-Data-Is-Stored]]
- [[Cloud sync|Cloud-Sync]]
- [[Backup & restore|Backup-and-Restore]]
- [[Export & import|Export-and-Import]]
- [[Migrating from another tool|Migrating-from-Another-Tool]]
- [[Storage triage|Storage-Triage]]
- [[Danger zone: erasing data|Danger-Zone-Erasing-Data]]

**Bridge & Home Assistant**
- [[Bridge overview|Bridge-Overview]]
- [[Running the bridge|Running-the-Bridge]]
- [[Bridge API tokens|Bridge-API-Tokens]]
- [[Home Assistant|Home-Assistant-Integration]]
- [[AI assistant query (MCP)|AI-Assistant-Query-MCP]]
- [[Webhooks]]
- [[Webhooks, MQTT & iCal|Webhooks-MQTT-and-iCal]]

**Settings & customisation**
- [[Appearance & theming|Appearance-and-Theming]]
- [[Branding|Branding]]
- [[Dashboard & widgets|Dashboard-and-Widgets]]
- [[Modular UI|Modular-UI]]
- [[Language & region|Language-and-Region]]
- [[Notifications|Notifications-Settings]]
- [[Kiosk & tablet mode|Kiosk-and-Tablet-Mode]]

**Reference**
- [[Command palette & shortcuts|Command-Palette-and-Shortcuts]]
- [[Calculations in number fields|Calculations-in-Number-Fields]]
- [[Glossary]]
- [[FAQ & troubleshooting|FAQ-and-Troubleshooting]]
- [[Privacy & security|Privacy-and-Security]]
- [[About & diagnostics|About-and-Diagnostics]]
