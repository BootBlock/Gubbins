...
| **Empty custom locations** | Your empty custom [[locations\|Locations-and-Stock]] only. Built-in system locations are kept, and so is any location still holding items or stock, or with a loan recorded against it — delete one of those from the [[Locations screen\|Locations-and-Stock]] instead, which returns any open loan first. |
