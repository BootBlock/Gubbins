...
Your records go in first and the full-resolution photos are written afterwards — which is exactly
when a device that's short on space runs out of it, because the restore has just written a second
full copy of your database.

If that happens the restore is **not** undone, because by then it has already taken effect: your
...
After a **Replace**, or a full-archive restore from the recovery screen, it also asks you to
**reload**: replacing the database file closes the one Gubbins was using, and reloading is what
picks the restored one up.
