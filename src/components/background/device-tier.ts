 ...
 * and sprite draw pays for every one of those pixels. A budget phone reports a ratio of 2.6–3 with
 * a GPU that is a fraction of a desktop's; the engine's own {@link DEFAULT_DPR_CAP} already holds
 * that to 2, but 2 is still four times the fill of a 1× display, spent on a decoration nobody is
 * looking closely at. Taking it to 1.5 more than halves that again, and it is the one lever that
 * scales the *whole* frame — clear, mound blit, haze stretch and every `drawImage` alike — rather
 * than trimming a single draw call.
 *
 ...
 */
import { COARSE_POINTER_QUERY } from '@/lib/env/device';
import { DEFAULT_DPR_CAP } from './precip-engine';

// Re-exported so a caller choosing a cap reads both ends of the range from one module, while the
// value itself still has exactly one definition — the engine's.
export { DEFAULT_DPR_CAP };
...
export interface DeviceTierHints {
  ...
  /** {@link COARSE_POINTER_QUERY} — a touch device, so likely a phone or tablet. */
 ...
 * desktop display still renders at {@link DEFAULT_DPR_CAP}.
 */
...
export function readDeviceTier(): DeviceTierHints {
  ...
  return {
    ...
    coarsePointer: typeof matchMedia === 'function' ? matchMedia(COARSE_POINTER_QUERY).matches : undefined,
  };
}
