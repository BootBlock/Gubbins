...
export function recordOnlyReceiptReason(trackingMode: TrackingMode): string | null {
  switch (trackingMode) {
    case 'SERIALISED':
      return 'each unit of a serialised item is tracked as its own record, so it has no counted quantity';
    case 'CONSUMABLE_GAUGE':
      return 'a consumable is measured by how full it is, not by a count of units';
    case 'UNTRACKED':
      return 'an untracked item is listed but never counted';
    case 'DISCRETE':
      return null;
    default:
      ...
      return 'this item has no counted quantity';
  }
}
