...
export interface HygieneItemFlags {
  ...
  /**
   * False when nothing prices the item: no manual current value (issue #706), no unit cost, no
   * preferred supplier cost and no purchase price for a counted item (issue #688), or no cost per
   * unit of measure for a gauge (#683).
   *
   * The list has to be exactly the one valuation prices from, or the screen sends the user to fix
   * something that is not broken. `ReportRepository` therefore resolves this through the
   * `hasValuationSource` seam that sits beside `valuedUnitValue` rather than restating the sources
   * — restating them is precisely how the current value came to be missing from this flag.
   */
...
> = {
  ...
  'missing-price': {
    label: 'Missing price',
    description: 'No current value, unit cost, supplier price or purchase price.',
