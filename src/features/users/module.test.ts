...
describe('turning the module off again (plan §3)', () => {
  it('returns the session to unrestricted/Admin, so nobody is left refused', () => {
    // On: the signed-in user's real (narrow) authority applies.
    useModulesStore.getState().setFeatureIntent('users', true);
    ...
    return refreshAuthority()
      .then((resolved) => {
        expect(resolved.authority).toEqual({ mode: 'granted', grants: new Set(['items:read']) });
        expect(resolved.actorId).toBe(RESTRICTED_USER.id);

        // Off again: the app must act as Admin immediately. A stale restricted authority here is
  ...
  it('does not read the database at all while the module is off', async () => {
    // Single-user mode is the overwhelmingly common case and must stay free — and, more to the
    // point, must not be able to fail on a database error and deny.
    const getById = vi.fn();
