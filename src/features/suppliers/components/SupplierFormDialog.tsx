...
import { useSupplierByName } from '../queries';
 ...
 *   dialog offers the merge instead, which is what the user actually meant. The collision is
 *   looked up against the **whole** table (issue #386) rather than a loaded page, so a clash
 *   with a supplier this screen has never shown is caught the same way as one it has.
 ...
 */
export function SupplierFormDialog({ supplier, onClose, onMerge, onAnnounce }: SupplierFormDialogProps) {
  const t = useT();
  ...
  // Looked up under the folded identity key — the same comparison the database's UNIQUE index
  // makes — so this catches exactly the collisions the write would have rejected, no more and
  // no fewer. A supplier that merely isn't on screen is still a collision, hence the query.
  const { data: existing } = useSupplierByName(trimmed);
  const clash = existing && existing.id !== supplier?.id ? existing : undefined;
