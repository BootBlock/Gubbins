 ...
 * full row set of every syncable table (keyed by table name); `tombstones` carries
 * the §7.2 deletions; `gaugeHistory` carries the net-value deltas the §7.3 Delta-CRDT
 * replays. Mirrors the §2 "Versioned JSON File" so a sync doc *is* a restorable backup.
 */
