 ...
 *
 * This file is the drift test that claim asks for. It reads the references out of the real migrated
 * schema and requires every one of them to be classified below, then *drives* each blocking and
 * cascading classification through a real erase rather than reading the predicate's source text: a
 * blocking reference must leave its location standing, and a cascading one must not. Each case
 * seeds its own reference and nothing else, so it fails when the clause it names goes, rather than
 * when a neighbouring clause does.
 */
...
/**
 * Drop the empty rows a seed drags along with it, leaving one reference standing alone.
 *
 * Stock references do not arrive singly: homing an item at a location opens an empty placement
 * *and* an empty batch beneath it, and inserting a batch derives a placement through the recompute
 * triggers. A case seeded that way keeps its location whichever of those three clauses is still in
 * the predicate — which is watching a neighbour hold the claim up, not driving it. Everything the
 * case is not about therefore goes, and because only empty rows are dropped, the tidy-up cannot
 * move stock that a later assertion counts.
 *
 * @param keep the table this case drives, whose rows at the van must survive the tidy-up.
 */
async function isolate(driver: MemoryDriver, keep: 'items' | 'stock_batches'): Promise<void> {
  if (keep === 'items') {
    await driver.execute('DELETE FROM stock_batches WHERE location_id = ? AND quantity = 0;', [VAN]);
  }
  await driver.execute('DELETE FROM item_stock WHERE location_id = ? AND quantity = 0;', [VAN]);
}
...
const REFERENCES: Readonly<Record<string, ReferenceCase>> = {
  ...
  'items.location_id': {
    kind: 'blocks',
    // Homing an item somewhere also opens an empty placement and batch there, so both go again
    // and the home pointer is left as the only thing naming the van.
    seed: async (driver) => {
      await new ItemRepository(driver).create({ name: 'Torque wrench', locationId: VAN });
      await isolate(driver, 'items');
    },
  },
  ...
  'stock_batches.location_id': {
    kind: 'blocks',
    // A batch derives a placement through the recompute triggers, so that placement goes again.
    seed: async (driver, itemId) => {
      await driver.execute(
      ...
      await isolate(driver, 'stock_batches');
    },
  },
...
describe('"Empty custom locations" and the references the schema declares', () => {
  ...
  it('keeps a location whose loan has already come back', async () => {
    // A return gives the units back and stamps `returned_at`, but leaves the borrower pointer
    // where it is, so the row stays a record of a loan the van was party to. Neither `checkouts`
    // clause asks whether the loan is open, so the van stays out of this target rather than having
    // that record cascaded away with no deletion marker — which is why the tooltip sends the user
    // to the Locations screen instead of promising that returning the loan is enough.
    const checkouts = new CheckoutRepository(driver);
    const items = new ItemRepository(driver);
    const checkout = await checkouts.checkout({ itemId, locationId: VAN, quantity: 2 });
    await checkouts.checkIn(checkout.id);
    expect((await items.getById(itemId))?.quantity).toBe(5);

    expect(await badgeCount()).toBe(0);
    expect(await eraseAndCountVan()).toBe(1);
    const closed = await driver.queryOne<{ n: number }>(
      'SELECT COUNT(*) AS n FROM checkouts WHERE location_id = ? AND returned_at IS NOT NULL;',
      [VAN],
    );
    expect(Number(closed?.n ?? 0)).toBe(1);
  });
});
