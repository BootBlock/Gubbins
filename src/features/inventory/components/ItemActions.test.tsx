...
describe('ItemActions — checkout gating (Phase 6)', () => {
  it('offers Check out on an eligible item when Contacts is on', () => {
    render(<ItemActions item={item} locations={[]} />);
    expect(screen.queryByLabelText('Check out')).not.toBeNull();
  });

  it('hides Check out when Contacts is off, leaving the other actions intact', () => {
