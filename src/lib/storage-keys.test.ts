...
import { EXPORTABLE_SETTING_KEYS } from '@/features/backup/backup-settings';
...
describe('namespaced storage-key registry', () => {
  ...
  it('agrees with the backup allow-list about which keys travel in a backup', () => {
    // The settings *groups* (issue #175) stay the authority for what a backup carries — they
    // also split the preferences blob and drive the picker. This registry records the same fact
    // per key, so the two must not disagree: adding a key to a group without a `backupIncluded`
    // decision here (or flagging one here that no group exports) fails the build instead of
    // silently changing what a shared backup file contains.
    expect([...backupIncludedKeys()].sort()).toEqual([...EXPORTABLE_SETTING_KEYS].sort());
  });

  it('pins the exact set of keys a portable backup may carry', () => {
    // Asserted exhaustively, not by exclusion, so widening what every shared backup file carries
    // has to be a deliberate, reviewed edit rather than a side effect of adding a store.
