 ...
 *  - `stock_delta_capture` — the local stock-apply switch row the {@link STOCK_DELTAS_TABLE}
 *    capture triggers and the derived-quantity recompute triggers consult (issues #188, #548).
 *    Pure device-local session state (a single row, holding whether capture is on, whether it is
 *    recording asserted counts, the key of any one-shot operation whose writes it is deriving ids
 *    for, and whether the stock projection recomputes per row); a peer's value would be
 *    meaningless and syncing it could wrongly suppress or double a capture.
 *
