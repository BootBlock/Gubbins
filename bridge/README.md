...
editing the item record. The **OData service** is the same reads under a different envelope, so
each of its entity sets needs exactly what its REST twin needs — `/api/v1/odata` is never the
cheaper door.
