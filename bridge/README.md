...
| `GUBBINS_BRIDGE_WEBHOOKS_FILE` | no | `webhooks.json` | Path to the **git-ignored** JSON webhook-target list. The target **secrets live only here** — never in a committed file. Point it outside the repo, or keep the name matching `webhooks*.json`: the `.gitignore` / `.dockerignore` rules match those names, so a file called anything else is neither kept out of git nor out of a Docker image. |
