...
> **Gubbins Inventory** in HACS and choosing the newest **version** rather than `main`.
...
`hacs.json` sets `hide_default_branch`, which keeps `main` out of the version list HACS offers.
JSON carries no comments, so the reason is recorded here: HACS tracks a repository by its
